...
null == r && ((r = function(e) {
    ...
    if (e in o) {
        ...
        return i[e] = a, t.call(a.exports, a, a.exports), a.exports
    }
    var r = new Error("Cannot find module '" + e + "'");
    throw r.code = "MODULE_NOT_FOUND", r
}).register = function(e, t) {
    o[e] = t
}, a.parcelRequire18bb = r), r.register("fH0PM", (function(t, a) {
    var i, o;
    ...
    var r = {};
    i = function(e) {
        for (var t = Object.keys(e), a = 0; a < t.length; a++) r[t[a]] = e[t[a]]
    }, o = function(e) {
        var t = r[e];
...
const B = J("#setup-os-select"),
    ...
    Y = e => {
        Z.map(((t, a) => t.classList.toggle("active", a == e))), W.classList.add("!opacity-0"), W.classList.add("translate-y-[10px]"), setTimeout((() => {
            W.innerHTML = V[e](j), W.classList.remove("!opacity-0"), W.classList.remove("translate-y-[10px]")
        }), 300), X.classList.toggle("hidden", 0 == e), G.classList.toggle("hidden", 4 == e), J("#setup-btn-wrap").classList.toggle("onlyprev", 4 == e)
    };
G.addEventListener("click", (() => {
    ...
    j = e.target.value, Y(Z.indexOf(J("#setup-stepper li.active")))
})), B.value = j, Z.map(((e, t) => e.addEventListener("click", (() => Y(t))))), Y(0);
let Q = (e, t, a) => (Object.keys(t).forEach((i => e = e.replace(new RegExp(`{{${i}}}`, "g"), a.indexOf(i) > -1 ? new URL(t[i], "file:///src/index.js") : t[i]))), e);
(async () => {
    ...
    let a = e.join(""),
        i = e.reverse().join("");
    N("#marquee1").forEach((e => e.innerHTML = a)), N("#marquee2").forEach((e => e.innerHTML = i)), $(), N(".marquee-inner").forEach((e => e.style.transform = "translateZ(0)")), J("#show-all-downloads").addEventListener("click", (() => {
        J("#all-downloads").classList.remove("hidden"), J("#all-downloads").classList.add("flex"), setTimeout((() => {
            J("#all-downloads").classList.add("show")
        }), 10)
    })), J("#close-all-downloads").addEventListener("click", (() => {
        J("#all-downloads").classList.remove("show"), setTimeout((() => {
            J("#all-downloads").classList.remove("flex"), J("#all-downloads").classList.add("hidden")
        }), 150)
    })), console.log(`Loaded apps marquee with ${e.length} items.`);
    let o = JSON.parse(localStorage.getItem("eventLogCache")) || [],
        r = [];
    o.length && localStorage.getItem("eventLogCacheDate") < Date.now() - 3e5 && (r = o), r.length || (r = await (await fetch("https://api.github.com/orgs/sidestore/events?per_page=100")).json(), localStorage.setItem("eventLogCache", JSON.stringify(r)));
    const n = JSON.parse(localStorage.getItem("reposCache")) || [];
    let s = [];
    n.length && localStorage.getItem("reposCacheDate") < Date.now() - 144e5 && (s = n), s.length || (s = await (await fetch("https://api.github.com/users/sidestore/repos?per_page=50")).json(), localStorage.setItem("reposCache", JSON.stringify(s)));
    ...
    let p = r.filter((e => Object.keys(c).includes(e.type))).filter((e => !e.actor.login.includes("[bot]"))).filter((e => "PullRequestEvent" !== e.type || ["opened", "closed", "reopened"].includes(e.payload.action))).map((e => ({
        ...
        if ("commit" == t.event) {
            let a = e[e.length - 1];
            if (a && "commit" == a.event && a.data.actor.login == t.data.actor.login && a.data.repo.name == t.data.repo.name) return a.data.commits += t.data.commits, a.data.date.lastDate = t.data.date.firstDate, e
        }
        return e.push(t), e
    }), []).map((e => {
        ...
        return t(0 == i ? "just now" : 1 == i ? "a minute ago" : i <= 60 ? `${i} minutes ago` : i / 60 == 1 ? "an hour ago" : i / 60 <= 24 ? `${Math.round(i/60)} hours ago` : i / 60 <= 48 ? "yesterday" : i / 60 <= 8760 ? `${Math.round(i/60/24)} days ago` : "a long time ago"), e
    })).map(((e, t) => {
        ...
        "commit" == e.event && (o += `pushed ${e.data.commits} commit${e.data.commits>1?"s":""}`), "issue_comment" == e.event && (o += `commented on issue ${i(e.data.issueID,"issues")}`), "pull_comment" == e.event && (o += `commented on pull request ${i(e.data.pullID,"pull")}`), "pull_request" == e.event && (o += ` ${e.data.action} pull request ${i(e.data.pullID,"pull")}`);
